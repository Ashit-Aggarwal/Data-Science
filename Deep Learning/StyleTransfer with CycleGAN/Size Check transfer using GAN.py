...
# We can see from above that the size of images provided is same for both **T1 and T2** type MRI scans which is 217,181. Also 181 is a prime number. So we would scale the data to 256x256

# As we have been provided with very few images we need to generate a few more images
# 
# #### Data Augmentation - to increase number of images
# 
...
T1_image = tf.keras.preprocessing.image_dataset_from_directory(pathlib.Path("MRI/TrainT1"),
                                                               labels=None,
...
T2_image = tf.keras.preprocessing.image_dataset_from_directory(pathlib.Path("MRI/TrainT2"),
                                                               labels=None,
                                                               color_mode='grayscale', # add channel to the image
                                                               batch_size=None,
                                                               image_size=(217,181),
...
def generator():
    input_model = tf.keras.layers.Input(shape=(64,64,1))
    ...
    upsample_model = [
        upsample(256, 4, apply_dropout=True),  # (batch_size, 2, 2, 512)
        upsample(256, 4, apply_dropout=True),  # (batch_size, 4, 4, 512)
        upsample(256, 4, apply_dropout=True),  # (batch_size, 8, 8, 512)
...
#     x = downsample(256, 6)(x)
    ...
    initializer = tf.random_normal_initializer(0,0.2)
    x = tf.keras.layers.Conv2D(256, 4, strides=1, kernel_initializer=initializer, use_bias=False)(x) # (batch_size, 31, 31, 256)

    x = tf.keras.layers.BatchNormalization()(x)

    x = tf.keras.layers.LeakyReLU()(x)

    x = tf.keras.layers.ZeroPadding2D()(x)  # (batch_size, 33, 33, 256)

    last = tf.keras.layers.Conv2D(1, 4, strides=1,
                                kernel_initializer=initializer)(x)  # (batch_size, 30, 30, 1)
